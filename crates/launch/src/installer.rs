...
use lighty_version::version_metadata::{Library, Native, Client, AssetsFile, VersionBuilder,Mods};
use lighty_core::hosts::HTTP_CLIENT as CLIENT;
use lighty_loaders::utils::sha1::verify_file_sha1;
...
impl<'a> Installer for Version<'a> {
    ...
    async fn install(&self , builder: &VersionBuilder) -> InstallerResult<()> {
        ...
        time_it!("Total installation", {
            ...
            tokio::try_join!(
                verify_and_download_libraries(self, &builder.libraries),
                //TODO revoir les unwraps
                verify_and_download_natives(self, builder.natives.as_deref().unwrap_or(&[])),
                verify_and_download_mods(self, builder.mods.as_deref().unwrap_or(&[])),
                verify_and_download_client(self, builder.client.as_ref()),
                verify_and_download_assets(self, builder.assets.as_ref()),
            )?;
        });
...
async fn create_directories<'a>(version: &Version<'a>) {
...
async fn verify_and_download_libraries<'a>(version: &Version<'a>, libraries: &[Library]) -> InstallerResult<()> {
...
async fn verify_and_download_mods<'a>(version: &Version<'a>, mods: &[Mods]) -> InstallerResult<()> {
...
async fn verify_and_download_natives<'a>(version: &Version<'a>, natives: &[Native]) -> InstallerResult<()> {
    if natives.is_empty() {
        return Ok(());
    }
...
async fn verify_and_download_client<'a>(version: &Version<'a>, client: Option<&Client>) -> InstallerResult<()> {
...
async fn verify_and_download_assets<'a>(version: &Version<'a>, assets: Option<&AssetsFile>) -> InstallerResult<()> {
